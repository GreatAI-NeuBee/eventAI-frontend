...
import { Save, Phone, Users, MapPin, Settings, Upload, FileText, ExternalLink, CheckCircle, Bot, ChevronDown, ChevronUp } from 'lucide-react';
...
interface VenueLayoutEditorProps {
  ...
  hideLayoutTypeControls?: boolean; // Hide layout type selection and SVG upload
}
...
const VenueLayoutEditor: React.FC<VenueLayoutEditorProps> = ({
  ...
  const toggleGateExpansion = (exitId: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <StaticGlassCard intensity="medium" blur="md">
        ...
        {/* Layout Type Selection - Hidden in EventDetails page */}
        {!hideLayoutTypeControls && (
          <>
            <div className="mb-6">
              ...
              <div className="border rounded-lg p-4 bg-white">
                <div className="flex items-center gap-4">
                  ...
                  <select 
                    ...
                    disabled={readOnly}
                    className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
                  >
      ...
      {localLayout.exitsList && localLayout.exitsList.length > 0 && (
        <StaticGlassCard intensity="medium" blur="md">
          ...
          <div className="space-y-4">
            {localLayout.exitsList.map((exit) => {
              ...
              const isExpanded = expandedGates.has(exit.id);
              
              return (
                <div key={exit.id} className="border border-white/10 rounded-lg overflow-hidden">
                  {/* Collapsible Header */}
                  <button
                    onClick={() => toggleGateExpansion(exit.id)}
                    className="w-full p-4 flex items-center justify-between hover:bg-white/5 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        isDarkBackground ? 'bg-white/10' : 'bg-blue-100'
                      }`}>
                        <Users className={`h-5 w-5 ${isDarkBackground ? 'text-white' : 'text-blue-600'}`} />
                      </div>
                      <div className="text-left">
                        <h5 className={`font-medium ${getTextColor()}`}>{exit.name}</h5>
                        <p className={`text-xs ${getSecondaryTextColor()}`}>
                           Capacity: {config.capacity || 800} people/hour
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {config.picPhoneNumber && isPhoneValid && (
                        <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-500/20 text-green-300 border border-green-400/30">
                          ✓ Configured
                        </span>
                      )}
                      {isExpanded ? (
